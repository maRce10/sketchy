...
#'
#' \code{git_ignore_files} generates the folder structure of a research compendium.
#' @usage git_ignore_files(name = "research_compendium", path = ".", force = FALSE, format,
...
git_ignore_files <- function(name = "research_compendium", path = ".", force = FALSE, format, comments = NULL, packrat = FALSE, git = FALSE, clone = NULL)
{
  ...
  if (any(basename(format) == "manuscript")){
    if (!file.exists(file.path(path, name, grep("manuscript$", format, ignore.case = TRUE, value = TRUE)[1], "manuscript.Rmd")))
    ...
    if (!file.exists(file.path(path, name, grep("manuscript$", format, ignore.case = TRUE, value = TRUE)[1], "apa.csl")))
